...
def validate_governance_dependency_map_mirror(path: Path = MIRROR) -> dict[str, Any]:
    ...
    if data["owner_repo"] != "logos-boundary-literature":
        raise MirrorValidationError(f"{_rel(path)} owner_repo must be logos-boundary-literature")
    ...
    for key in (
        ...
        "local_repo_may_treat_boundary_need_as_governance_authority",
    ):
    ...
    for rel in enforcement.values():
        if isinstance(rel, str) and rel.endswith((".md", ".py", ".yml", ".yaml")) and not (ROOT / rel).exists():
    ...
    for surface, phrases in REQUIRED_SURFACE_PHRASES.items():
        ...
        missing_phrases = [phrase for phrase in phrases if phrase.lower() not in text]
        if missing_phrases:
